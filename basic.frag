// frag shader code is based on  code from Daniel Shiffman, Inigo Quelez, and Martijn Steinrucken (aka the Art of Coding), and Jamie Wong
...
float mandelbulb( in vec3 pos, out vec4 col) 
{
  vec3 zeta = pos;
  float m = dot(pos,pos);
...
float sceneSDF(vec3 samplePoint) {
  vec4 edge;
  return mandelbulb( samplePoint, edge );
}
